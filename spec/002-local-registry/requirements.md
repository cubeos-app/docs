...
Source: `architecture/08_LOCAL_REGISTRY.md` + Article III (Offline-first) + Article XIV (Local registry only for coreapps).
...
REQ-202: The system shall persist registry data at `/cubeos/data/registry/`.
REQ-203: While the system is operating, the registry shall be the only image source consulted at coreapp pull time.
REQ-204: The system shall populate the local registry with all coreapp images at image-build time.
REQ-205: The system shall pin every coreapp image to a SHA-256 digest in the compose files.
...
REQ-208: When an imported image is added, the system shall record source filename and import timestamp to `/cubeos/data/registry/imports.log`.
REQ-209: The system shall NOT delete imported images automatically.
REQ-210: While the system is in `offline_hotspot` mode, the system shall NOT attempt any pull from a remote registry.
REQ-211: If a coreapp's compose file references an image not present in the local registry, then the system shall fail the deploy with an actionable error listing the missing image.
